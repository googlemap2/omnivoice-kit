...
async function extractVideoPreviewFromUrl(src: string): Promise<VideoPreviewInfo> {
  const emptyPreview = {
    previewUrl: "",
    width: 1280,
    height: 720,
  ...
  return new Promise((resolve) => {
    ...
    const finish = (previewInfo = emptyPreview) => resolve(previewInfo);
...
export const Uploads = () => {
  ...
  const handleAddVideo = async (video: any) => {
    ...
    const previewInfo = video.metadata?.previewUrl
      ? {
      ...
      : await extractVideoPreviewFromUrl(srcVideo);
