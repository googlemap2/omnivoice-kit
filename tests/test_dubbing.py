...
from voicekit.subtitles import SubtitleSegment
from voicekit.translation import TranslationResult, TranslationSegment
...
class DubbingAudioTests(unittest.TestCase):
    ...
    def test_dub_file_smoke_with_short_audio_fixture_and_mocked_backends(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("voicekit.dubbing.get_profile_store") as get_profile_store,
                patch("voicekit.dubbing.extract_audio", side_effect=fake_extract_audio),
                patch("voicekit.dubbing.transcribe_file", return_value=SimpleNamespace(language="en")),
                patch("voicekit.dubbing.from_transcription_result", return_value=subtitle_segments),
                patch("voicekit.dubbing.translate_segments", return_value=translated),
                patch(
                    "voicekit.dubbing.generate_clone_with_speaker_id",
                    return_value=((24000, np.ones(12000, dtype=np.int16)), "ok"),
                ) as generate_tts,
                patch("voicekit.dubbing.has_video_stream", return_value=False),
            ):
