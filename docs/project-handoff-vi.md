...
- Python package: `voicekit/`
- API: FastAPI trong `voicekit/api.py`
- CLI: `voicekit/cli.py`, chạy bằng `uv run voicekit ...`
- TTS runtime: `omnivoice`, PyTorch
- Audio IO/DSP: `soundfile`, `numpy`, `torchaudio`/DSP helper trong `voicekit/audio.py`
- ASR: `faster-whisper`
- Video/audio processing: FFmpeg wrapper trong `voicekit/media.py`
...
- Function: `generate_clone_with_speaker_id()` trong `voicekit/core.py`
...
```bash
uv run voicekit speaker-id --speaker_id yen --text "Xin chào" --output out.wav
```
...
- Gọi `generate_clone_with_ref_audio()` trong `voicekit/core.py`.

CLI:

```bash
uv run voicekit ref-audio --ref_audio path/to/ref.wav --ref_text "..." --text "..." --output out.wav
```
...
- `generate_voice_design()` trong `voicekit/core.py`
- Bắt buộc có `instruct_items`.

CLI:

```bash
uv run voicekit voice-design --text "..." --instruct-item female --instruct-item middle-aged --output out.wav
```
...
```bash
uv run voicekit transcribe --input path/to/audio.wav --language vi --format verbose_json
```
...
```bash
uv run voicekit subtitle-import --input transcript.srt --output segments.json
uv run voicekit subtitle-export --input segments.json --format vtt --output transcript.vtt
```
...
```bash
uv run voicekit translate --text "Xin chào" --source-language vi --target-language en --provider passthrough
```
...
- `voicekit/dubbing.py`
- FFmpeg helper: `voicekit/media.py`
...
```bash
uv run voicekit diarize --input path/to/audio.wav --output speakers.json
```
...
python -m voicekit.scripts.backup_model ...
python -m voicekit.scripts.verify_checksum ...
```
...
5. Với backend, đọc `voicekit/api.py` endpoint trước, sau đó đi vào module feature tương ứng.
