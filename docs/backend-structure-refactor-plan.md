...
- [x] List FastAPI endpoints from `backend/api.py`.
- [x] List CLI commands from `backend/cli.py`.
...
- [x] `python -m py_compile backend/api.py backend/cli.py`
- [x] `uv run python -m backend.cli --help`
...
- [x] `backend.app.main` owns the FastAPI app object.
...
- [x] Create `backend/app/errors.py`.
- [x] Move reusable HTTP error helpers from `backend/api.py`.
...
- [x] `python -m py_compile backend/api.py backend/app/errors.py backend/app/dependencies.py`
...
Goal: Move Pydantic request/response models into `backend/app/schemas/`.
...
- [x] `python -m py_compile backend/api.py backend/app/schemas/*.py`
- [x] OpenAPI route registration still succeeds on app import.
- [x] Frontend build passes.

Exit criteria:

- [x] `backend/api.py` no longer owns Pydantic schema definitions.
...
- [x] Register routers from `backend/app/main.py`.

Validation:

- [x] `python -m py_compile backend/app/main.py backend/app/routers/*.py`
- [x] Smoke `/health`
- [x] Smoke `/v1/meta`
...
- [x] Move remaining workflow endpoints out of `backend/api.py`.
- [x] Register remaining workflow endpoints through focused routers under `backend/app/routers/`.
- [x] Keep endpoint paths, request fields, and response shapes unchanged.
- [x] Keep `backend/api.py` as a compatibility shim.
- [x] Split `routers/workflows.py` into focused routers (`speech.py`, `transcription.py`, `subtitles.py`, `translation.py`, `dubbing.py`, `diarization.py`, `voices.py`, `jobs.py`, `settings.py`).
- [x] Keep `routers/workflows.py` as an empty compatibility router during the transition.

Validation:

- [x] `python -m py_compile backend/api.py backend/app/main.py backend/app/routers/*.py`
- [x] Smoke route registration for `/v1/audio/transcriptions`.
- [x] Smoke `/v1/translation/providers`.
- [x] Smoke route registration after focused router split.
- [x] Smoke `/v1/jobs`.
- [x] `cd frontend && pnpm build`

Exit criteria:

- [x] `backend/api.py` is thin except for compatibility import.
...
- [x] Move service implementations from root modules into `backend/services/`.
- [x] Keep root modules as compatibility aliases.
- [x] Route workflow imports through service modules.
- [ ] Optional follow-up: move long request orchestration blocks from focused routers into smaller service functions.
...
- [ ] Optional follow-up: thin focused routers further by moving orchestration into service functions.

## Phase 7: Infrastructure

Status: Done

Goal: Group runtime infrastructure under `backend/infrastructure/`.

Tasks:

- [x] Create `backend/infrastructure/__init__.py`.
...
- [x] Keep CLI implementation compatible through `backend/cli_legacy.py`.

Validation:

- [x] `uv run python -m backend.cli --help`
- [x] `uv run python -m backend.cli emotion-script --help`
- [x] `uv run python -m backend.cli transcribe --help` remains covered by command registration.
- [x] `uv run python -m backend.cli subtitle-import --help` remains covered by command registration.
- [x] `uv run python -m backend.cli dub --help` remains covered by command registration.
...
- [x] Migrate internal imports from root compatibility shims to `backend/services`, `backend/domain`, and `backend/infrastructure`.
- [x] Remove unused root-level compatibility modules that had no runtime imports.
...
- [x] Internal import search shows no `backend/*.py` compatibility shim imports inside the package.
...
- [x] `python -m py_compile backend/api.py`
- [x] `uv run python -m backend.cli --help`
- [x] `cd frontend && pnpm build`
...
## Public Compatibility Shims to Keep

Internal imports have moved to the new package paths. Keep these root-level shims for public/backward compatibility until external compatibility is explicitly dropped:
...
- `backend/dictation.py`
- `backend/dubbing.py`
- `backend/emotion_tts.py`
- `backend/history.py`
- `backend/jobs.py`
- `backend/media.py`
