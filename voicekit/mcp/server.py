...
from voicekit.core import OMNIVOICE_LANGUAGE_CHOICES, generate_clone_with_speaker_id, get_profile_store
...
def generate_speech_tool(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    output_path = Path(str(arguments.get("output_path") or "outputs/mcp/speech.wav"))
...
def list_resources() -> list[dict[str, Any]]:
    return [
        {
            "uri": "voicekit://generation-history/recent",
...
def read_resource(uri: str) -> dict[str, Any]:
    if uri != "voicekit://generation-history/recent":
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.http:
        ...
        uvicorn.run("voicekit.mcp_server:http_app", host=args.host, port=args.port)
        return
    serve_stdio()
